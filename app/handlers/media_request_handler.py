...
class MediaRequestHandler:
    ...
    def __init__(self, output_repository, media_service, image_service, chapter_service, metadata_service, job_service, filesystem_service) -> None:
    ...
    def process_create(self, request: CreateMediaRequest) -> dict:
        ...
        if request.poster_url:
            self._image_service.process_poster(request.poster_url, final_dir, request.poster_crop_settings)
    ...
    def process_update(self, request: UpdateMediaRequest) -> dict:
        ...
        if request.poster_url:
            self._image_service.process_poster(request.poster_url, target.directory, request.poster_crop_settings)
        elif request.local_poster_file:
            poster_path = self._output_repository.resolve_poster_file(request.output_name, request.local_poster_file)
            self._image_service.process_local_poster(poster_path, request.poster_crop_settings)
        if request.background_url:
            self._image_service.process_background(request.background_url, target.directory)
        return {"output_name": target.output_name, "video_path": str(target.mkv_path) if target.mkv_path else None}
